...
namespace Assignment1a.Objects
{
    class LevelBuilder : CollidableObject
    {
        ...
        public void Update(Rectangle border, List<CollidableObject> drawables, Rectangle[] borderBlocks)
        {
                ...
                previousGamePadState.Buttons.A == ButtonState.Released)
            {
                if (blocks > 0)
                {
                    activeObject = new CollidableObject(sprite, position, scale, Color.Black, 0.9f, 0f, true);
                }
            }
            ...
            {
                DropBlock(drawables, borderBlocks);
            }
        ...
        void DropBlock(List<CollidableObject> drawables, Rectangle[] border)
        {
            if (activeObject != null)
            {
                ...
                if (drop)
                {
                    ...
                    {
                        if (block.Intersects(activeObject.Rect))
                        {
                            drop = false;
                            break;
                        }
        ...
        public void DrawHUD(SpriteBatch spriteBatch, SpriteFont font1)
        {
            ...
            spriteBatch.DrawString(font1, "x" + blocks.ToString(), blockStringPosition, Color.Black, 0f, Vector2.Zero, scale, SpriteEffects.None, 1f);
        }
    }
}
